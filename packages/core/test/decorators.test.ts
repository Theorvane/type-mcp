...
describe("MCP decorators", () => {
	...
	it("does not inherit a parent class's component metadata", () => {
		...
		expect(
			getMcpServerDefinition(ParentServer)?.tools.map((tool) => tool.name),
		).toEqual(["parentTool"]);
		expect(
			getMcpServerDefinition(ChildServer)?.tools.map((tool) => tool.name),
		).toEqual(["childTool"]);
	});
	...
	it("returns frozen copied containers while preserving Zod schema identity", () => {
		...
		expect(
			Reflect.defineProperty(input, mutationMarker, {
				configurable: true,
				value: true,
			}),
		).toBe(true);
		expect(
			Reflect.has(
				getMcpServerDefinition(CopiedServer)?.tools[0]?.input ?? {},
				mutationMarker,
			),
		).toBe(true);
