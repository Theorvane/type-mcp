...
describe("LangChain current-facing documentation contract", () => {
	it("documents the tools-only LangChain and consumer-owned LangGraph integration boundary", async () => {
		...
		expect(combined).not.toMatch(
			/(?:0\.2\.0.*release[- ]target|release[- ]target.*0\.2\.0|release[- ]target boundary|verify npm publication|before installing, confirm the version exists|after publication, verify the npm version before treating it as installable|after the release version is visible in npm|Public `Theorvane\/type-mcp` is the delivery target|Repository target: public `Theorvane\/type-mcp`|repository API target|planned product capabilities)/i,
		);
