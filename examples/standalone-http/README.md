...
> **Published package example:** `@theorvane/type-mcp@0.2.2` provides `createMcpServer()` and `@theorvane/type-mcp/http`. This example leaves route hosting, durable session policy, and authorization to the application; the adapter owns in-process MCP session routing.
